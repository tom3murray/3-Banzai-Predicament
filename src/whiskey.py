...
os.chdir('C:/Users/Lyle_/Desktop/4 Yr Sem 2/DS/Banzai-predicament/data/')
os.getcwd()
...
regions_total = total_df.groupby(["Region", "Year"])["Total Cases"].sum()
regions_total = regions_total.to_frame().reset_index()


# Create dataframes grouped by income
incomes_total = total_df.groupby(["Income", "Year"])["Total Cases"].sum()
...
for region in unique_regions:
    ...
    # Create a date range for forecasted values starting from 2017 to 2024
    forecast_date_range = pd.date_range(start='2017', end='2024', freq='YS')
...
for region in unique_regions:
    ...
    plt.plot(test['Cases'], label='Actual Data (2012-2017)', color='green')
    plt.plot(forecast_data['Cases'], label='Forecast (2012-2017)', linestyle='--', color='red')

    # Set titles and labels
    plt.title(f"{region} Cases: 2012-2017 Forecast vs. Actual")
    plt.xlabel('Year')
    plt.ylabel('Cases')
    plt.legend()
...
for region in unique_regions:
    ...
    plt.plot(test['Cases'], label=f"{region} Actual Data (2012-2017)", color=colors[region][1], linestyle='--')
    plt.plot(forecast_data['Cases'], label=f"{region} Forecast (2012-2017)", color=colors[region][2], linestyle=':')

# Set titles and labels
plt.title("Cases by Region: 2012-2017 Forecast vs. Actual")
plt.xlabel('Year')
plt.ylabel('Cases')
